...
Every impl in this crate is generated by
[batch-impl](https://docs.rs/batch-impl) — a proc-macro DSL for generating
trait impls in bulk. The whole ~900-impl matrix is ~80 DSL blocks; each
method body is written exactly once, and the matrix supplies the quantity
(types × operators × arities). The few genuinely bulky algorithms (the
Hamiltonian product, extended euclid) are hand-written and clearly marked.
...
[docs/DESIGN.md](docs/DESIGN.md); if batch-impl can express a matrix that
